...
var AutoCrop = AutoCrop || function (options) {
    this.options = extend({}, AutoCrop.DEFAULTS, options);
}
...
AutoCrop.crop = function(image, options, callback) {
    // work around images scaled in css by drawing them onto a canvas
    if (image.naturalWidth && (image.naturalWidth != image.width || image.naturalHeight != image.height)) {
        var c = new AutoCrop(options).canvas(image.naturalWidth, image.naturalHeight),
